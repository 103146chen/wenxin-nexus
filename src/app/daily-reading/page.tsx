...
export default function DailyReadingPage() {
  ...
  // 使用 Store 中的 articles 進行判斷
  const allCompleted = articles.length > 0 && articles.every(article => 
  ...
  const handleRepair = () => {
      if (repairStreak()) alert('🛡️ 連勝已修復！');
      else alert('❌ 道具不足。');
  };

  const handleAcceptBreak = () => {
      if(confirm('確定要放棄連勝紀錄？')) acceptStreakBreak();
  };
                    ...
                    return (
                        <Link key={article.id} href={`/daily-reading/${article.id}`} className={`group flex items-center justify-between p-6 rounded-2xl border transition-all ${isDone ? 'bg-slate-50 border-slate-200 opacity-80' : 'bg-white border-slate-200 hover:border-indigo-400 hover:shadow-md'}`}>
                            <div className="flex items-center gap-6">
                                <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-bold text-lg ${isDone ? 'bg-slate-200 text-slate-500' : 'bg-indigo-100 text-indigo-600'}`}>{index + 1}</div>
                                <div><h3 className="text-lg font-bold text-slate-800 group-hover:text-indigo-700">{article.title}</h3><p className="text-sm text-slate-500">{article.author}</p></div>
                            </div>
